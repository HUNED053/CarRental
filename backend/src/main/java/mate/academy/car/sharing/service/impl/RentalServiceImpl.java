...
public Rental add(Rental rental) {
    ...
    // Check if rentalDate is set in the incoming rental request
    if (rental.getRentalDate() == null) {
        rental.setRentalDate(LocalDateTime.now()); // Set to current date if not provided
    }
    
    // Ensure that the returned rentalDate is in proper format
    rental.setRentalDate(rental.getRentalDate().withHour(0).withMinute(0).withSecond(0).withNano(0)); // Adjusting to ensure it's set correctly to the start of the day, if needed.
